...
视图。现在发送交给独立 goroutine，中间是一个有界队列（256）：队列满时不丢通知，对应的 watch
仍是 dirty，下一个 tick 会再次投递；同一个 URI 不会在队列里堆重复项。回归覆盖「队列填满后
仍能接收并记录新变更」和「重复 tick 不堆积同一 URI」，把入队改回内联即失败。

**仍未解决（上游）**：单个订阅者之间的投递仍是串行的——SDK 的广播按会话依次发送，cloudfs
拿不到按会话投递的入口，所以一个卡住的客户端仍会延迟**其他订阅者**收到通知，只是不再延迟
变更检测本身。另外 SDK 的 `Server.disconnect` 只从每个 URI 的内层 map 里删掉该会话，
**不会删除随之变空的外层 URI 条目**（`go-sdk v1.7.0` `mcp/server.go`），长期运行下
`resourceSubscriptions` 会按历史订阅过的不同 URI 数量增长。cloudfs 侧的簿记已有回归证明
会话反复连断后 `sessions`/`streams`/`count`/投递队列全部归零，但外层条目在 SDK 内部，
