...
高负载/慢 HTTP 客户端的长期运行仍待验收：VFS 事件有界且不阻塞文件 IO，但 SDK 的
广播发送可等待慢连接，尚不能宣称对其他客户端的通知延迟完全隔离。SDK 在旧版会话
断开后还可能保留空的 URI 容器，其长期内存回收也需处理。真实 FUSE/网盘事件仍需验收。

## Agent 使用建议

**搜索只覆盖已知目录树。** `query` 不含 `/` 时匹配名称；含 `/` 时按完整虚拟路径的字面子串匹配（例如 `work/src/`），不会把 `%`、`_` 当通配符。匹配忽略大小写，权限路径仍区分大小写。结果按路径深度、路径排序。目录改名后无需重写子树索引，查询在一个数据库快照内重建路径。1～2 个字符使用独立短字符串索引；较长查询同时检查尚未完成后台索引的记录，大量待索引记录或宽泛路径查询仍有扫描／排序成本，不能保证任何查询都快。
