...
`test/e2e` 用 1000 部影片（每部一个目录，内含影片与一张海报）的库做了两条断言：

- **冷扫描的后端调用数恰好等于把整棵树枚举一遍**（本例 1501 次，含分页），
  期望值由 provider 的分页大小推导而非写死。超出就说明遍历在重复列举目录——
  那正是把一次媒体库扫描变成 O(n²) 流量的原因。
- **扫描期间零 `ReadRange`、零 `DownloadURL`**：`.strm` 是一个指针，为了写它去打开
  影片内容会把库扫描变成全量镜像。
- **`dir_ttl: 24h` 下的重复扫描是 0 次后端调用**，且 0 次写入、全部 `unchanged`。
  这是刮削器按计划重复扫描时的实际代价。
- `--prune` 只删自己生成的东西：上游消失的片子对应的 `.strm` 被删除，刮削器写在
  旁边的 `.nfo` 保留。

这些是 fake provider 上的调用计数断言，不是真实网盘或真实播放器的验收。
