...
第一次使用、还没有账号和配置，先看 [从零开始](getting-started.md);下面是配置文件长什么样。
...
**放置。** 候选成员 = 在服务中、能拼出这个名字(见下)、有空间的成员;已经持有该路径的优先(覆盖写落在原地),然后按剩余空间(权重加成)、声明顺序。同一成员永远不放两份。v2 规划的按路径规则、成员 class、故障域与配额满换盘见 [pool-v2.md](pool-v2.md) §6。
