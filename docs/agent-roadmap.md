...
[界面计划](ui-plan.md) 阶段 F · 三期项由 [Agent-first 设计](agent-first-design.md)（P5，T-46 ~ T-57）吸收并重排，见其 §12。
...
- **stdio→HTTP 桥（T-43 结论：并存有数据风险，C0.5 已用写栅栏封住；桥排在三期其他条目之前，落地后关闭 T-43）**。→ T-50（2026-09-16 完成，T-43 已关闭）
- 递归删除目录的逐文件前像，按文件数与字节预算记录。→ T-48（P0，仅 cached，2026-09-16 完成）
- 控制台 `/fs/*` 与 WebDAV 操作进审计。→ T-51（经 `changes` 表，2026-09-16 完成）
- `_meta["cloudfs/session"]` 显式会话。→ 未吸收，保留
- `pull_events` MCP 工具。→ T-52（2026-09-16 完成）
- 纯 Go HNSW，仅在实测 `max_chunks` 规模下 p95 检索 > 200 ms 时实施。→ 未吸收，保留
- 团队场景：principal 的 `owner` 字段。→ T-56（P2）
- 多选文件发送给 Agent。→ 未吸收，保留
