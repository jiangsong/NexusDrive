...
var zh = map[string]string{
	...
	"err.restart_busy_cwd":              "拒绝重启：仍有进程的工作目录位于挂载内：%s；请先离开或关闭该目录后重试",
	...
	"agent.instructions.search":    "优先 context_search，按知识/记忆/交接分组并返回供 expected_version 读取的版本。coverage 的 listed < known 时先 warm 再搜。",
	...
	"agent.instructions.memory":    "memory_* 存在 %s；不确定信息先 memory_propose，confirm=true 的 memory_review 才持久化；agent=personal 跨智能体共享。",
	...
	"agent.prompt.search.steps":     "先用限定路径的 context_search。若 coverage 显示有未列举目录，对该路径 warm 后再搜；读取命中时把其 version 作为 expected_version。",
	...
	"agent.prompt.finish.session":   "现在调用 finish_session，summary 写清改了什么，并给下一智能体一段简短 handoff；handoff.md 与改动路径会进入 artifacts。",
