...
var fieldsEN = map[string]string{
	...
	"creds.quark.note":         "the cookie from a browser session; this is an unofficial API, so rate limits are conservative",
...
var fieldsZH = map[string]string{
	...
	"creds.quark.note":         "浏览器登录后的 cookie；这是非官方接口，限流更保守",
