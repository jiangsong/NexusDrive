...
var en = map[string]string{
	...
	"err.app_secret_required":           "this account still needs the OAuth application secret from its registration",
	"err.app_secret_not_used":           "a %s account does not authorize with an application secret",
	"err.app_secret_invalid":            "the application secret must be 1..4096 characters on a single line",
	"err.app_secret_store_failed":       "the application secret could not be stored; see the daemon log",
	"err.account_changed":               "the account changed while this was being saved; reopen it and try again",
	...
	"err.q_required":                    "q or a filter is required",
	"err.search_param":                  "search parameter %s is invalid",
	"err.restart_in_progress":           "a restart is already in progress",
	"err.restart_busy_cwd":              "restart refused because a process is working inside the mount: %s; leave or close that directory and retry",
	...
	"agent.instructions.search":    "Use context_search first: it groups mounted knowledge, memory and handoffs and returns versions for read_text expected_version. Its name search covers only listed directories; when coverage listed < known, call warm on the subtree and search again.",
	...
	"agent.instructions.memory":    "memory_* tools keep Markdown under %s. Put uncertain facts through memory_propose; only memory_review with confirm=true makes them durable. Use agent=personal for cross-agent memory.",
	...
	"agent.prompt.search.steps":     "Start with context_search restricted to the path. If coverage shows unlisted directories, call warm and search again; read a result with its version as expected_version.",
	...
	"agent.prompt.finish.session":   "Call finish_session now with a summary and a concise handoff for the next agent; handoff.md and changed paths become artifacts.",
