...
// localAttrWritable refuses a change inside a read-only subtree. The mode
// means nothing in it changes, and an attribute held beside the node is still
// a change to the file as the person sees it — the Finder shows tags and the
// quarantine flag, and ls(1) shows the permission bits.
func (f *FS) localAttrWritable(ctx context.Context, ino uint64) error {
